    ...
    try
      ...
      elseif b:fugitive_type ==# 'commit'
        ...
        if b:fugitive_display_format
          ...
          let cmd = [dir, 'show', '--no-color', '-m', '--first-parent', '--pretty=format:tree%x20%T%nparent%x20%P%nauthor%x20%an%x20<%ae>%x20%ad%ncommitter%x20%cn%x20<%ce>%x20%cd%nencoding%x20%e%n%n%s%n%n%b']
          if exists('g:fugitive_diff_context')
            let cmd += ['-U' . g:fugitive_diff_context]
          endif
          let cmd += [rev]
          call s:ReplaceCmd(cmd)
